...
* [x] **智慧即時比賽狀態監控（Smart Live Game Detection）**：
  * **目標**：利用外部公開的 ESPN Scoreboard API 即時監控 NBA 當日賽事狀態，動態阻擋或放行戰績更新與查詢，取代原先的「硬性 14:00/15:00 時間限制」。
  * **邏輯設計**：
    1. 當使用者查詢「本日戰績」（`#戰績`）或「當前進行週戰績」（例如 `#戰績W10`）時，若今日戰績圖檔已存在於快取，直接放行並回傳圖片（首關快取放行）。
    2. 若快取未命中，系統會發送請求至 ESPN Scoreboard API，解析今日所有比賽狀態（`pre`/`in`/`post`）。
    3. **若有比賽進行中（`in`）**：阻擋並提示 `目前仍有比賽正在進行`。
    4. **若所有比賽均未開始（`pre`）**：阻擋並提示 `今日比賽尚未開始`。
    5. **若有比賽結束但部分比賽尚未開始**：阻擋並提示 `今日比賽尚未全部結束，請在所有比賽結束後再進行查詢`。
    6. **若所有比賽已結束（`post`）或當日無賽事**：直接放行更新。
    7. **容災降級**：若外部 API 發生連線異常或超時（3秒），自動退回原有的台北時間時段限制（夏令 14:00 / 冬令 15:00）作為安全保障。
