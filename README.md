...
* [x] **實時一對一對戰數據查詢（Real-time One-on-One Matchup Query）**：
  * **指令**：`#對戰 [暱稱]` (例如：`#對戰 玩家A`)
  * **目標**：能夠自動識別該玩家當週的 Matchup 對手，並一次性回傳雙方的 9-Cat 累計數據與即時比分。
  * **邏輯設計**：
    1. **玩家 ID 反查**：透過 `team_mapping.json` 將輸入的玩家暱稱反查出對應的 `team_id`。
    2. **調用 Scoreboard API**：發送單次請求至 Yahoo Scoreboard 接口 `league/{league_id}/scoreboard;week={week}`。
    3. **自動對手匹配**：在回傳的 `matchups` 中尋找包含該 `team_id` 的對戰組別，並自動提取出對戰對手（Opponent）的 `team_id`、中文暱稱及兩隊的 9-Cat 統計數據。
    4. **對稱卡片渲染**：將兩隊的數據（如 FGM/A, PTS, REB 等）以左右並排、精緻對稱的 Flex Message 卡片回傳至 LINE 群組，並用粗體加強標註各項目的領先方，讓群組玩家能夠即時追蹤對決現況！
