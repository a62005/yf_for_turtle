...
1. **超級管理員 (Super Admin)**：
   * 僅限一位，擁有系統最高權限。
   * 可新增全域的「聯盟管理員 (Manager)」，並設定其方便識別的**識別名稱**（透過 60 秒互動對話）。
   * 自動擁有所有聯盟的管理員與白名單權限。
2. **聯盟管理員 (Manager)**：
   * 由超級管理員指派。
...
*   **功能**：廢除舊有帶參數的指令，重構為透過對話新增：
    *   **`#新增管理員`**：觸發 60 秒互動對話會話，將目標加入 `managers.json` 並綁定識別名稱。
...
### 6.1 超級管理員新增管理員 (對話會話)
1. **觸發指令**：超級管理員在對話框中輸入 **`#新增管理員`**。
2. **會話建立**：`session_manager` 設定狀態為 `add_manager`。
3. **提示訊息**：回覆：「`請在 60 秒內輸入欲新增的管理員 LINE ID 與方便識別的名稱（中間以空格分開，例如：U123456... 小明），或輸入 # 取消：`」。
4. **意圖攔截**：`IntentRouter` 攔截後續文字輸入：
   ...
   * 若輸入符合格式（例如：`U[a-fA-F0-9]{32}\s+\S+`）：
     ...
     * 回覆：「`✅ 成功將管理員 {display_name} ({target_id}) 加入全域管理員名單。`」。
     * 清除會話。
   * 若格式不符，提示：「`⚠️ 格式錯誤。請重新輸入（例如：U123456... 小明），或輸入 # 取消`」。

### 6.2 聯盟管理員新增白名單成員 (對話會話)
...
2. **新增與移除指令測試**：
   * 驗證 `#新增管理員` 對話會話中輸入 `LINE_ID 自定義名稱` 能正常解析並寫入 `managers.json`。
